 ...
 *  4. ROI 内 8x8 网格做局部 SAD 块匹配，得到每块 dx/dy；过滤低纹理块、MAD 异常块。
 *  5. signedDy = median(dy_i)，对 dy 做泄漏积分得到 position(t)。
 *  6. 最近 ~6s position(t) 滑窗 → 去趋势 → 自相关主频估计（默认 0.95–1.85 Hz）。
 *  7. 输出 {@link VideoWaveResult}（freqHz / confidence / position01 / motionEnergy 等）。
 *
...
public class VideoMotionWaveEstimator {
    ...
    public synchronized void pushFrame(long timestampMs, Bitmap frame, float[][] keypointsNorm) {
        ...
        if (haveMatch) {
            ...
            signedDy = br.medianDy;
            motionEnergy = br.energy;
        ...
        if (haveMatch && validBlocks >= minBlocks) {
            rawPosition = P.leak * rawPosition + signedDy;
        } else {
            // 块匹配数据不足：仅做衰减，不引入噪声
        ...
        if (posCount >= P.minWarmSamples && framesSinceEst >= P.estimateEveryNFrames) {
            ...
            VideoWaveResult result = doEstimate(timestampMs, rx, ry, rw, rh,
                    signedDy, motionEnergy, validBlocks, totalBlocks);
            latest.set(result);
            ...
            upd.debugInfo = String.format(java.util.Locale.US,
                    "[warm] roi=(%d,%d,%d,%d) blk=%d/%d sDy=%.2f rawP=%.2f mE=%.3f posN=%d",
                    rx, ry, rw, rh, validBlocks, totalBlocks, signedDy, rawPosition, motionEnergy, posCount);
            latest.set(upd);
        }
    }
    ...
    private VideoWaveResult doEstimate(long timestampMs,
                                       ...
                                       float signedDy, float motionEnergy,
    ...
    private BlockResult blockMatch() {
        ...
        // 3) 取中位数 dy
        float[] vd = new float[validCount];
        int j = 0;
        for (int i = 0; i < total; i++) if (valid[i]) vd[j++] = dys[i];
        Arrays.sort(vd);
        float median = vd[validCount / 2];

        // 4) MAD 异常剔除后再算一次中位数
        for (int i = 0; i < validCount; i++) _scratchAbs[i] = Math.abs(vd[i] - median);
        ...
        for (int i = 0; i < total; i++) {
            if (valid[i] && Math.abs(dys[i] - median) <= thresh) {
                filtered[fc++] = dys[i];
            }
        }
        if (fc == 0) {
